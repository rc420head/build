...
- crbremote: Add git remote for matching AquariOSBeta repository.
- crremote: Add gerrit remote for matching AquariOS repository.
...
function crbremote()
{
    ...
    PROJECT=`pwd -P | sed s#$ANDROID_BUILD_TOP/##g`
    PFX="android_$(echo $PROJECT | sed 's/\//_/g')"
    git remote add crb git@github.com:AquariOSBeta/$PFX
...
function crremote()
{
    ...
    CRUSER=`git config --get review.review.aquarios.org.username`
    if [ -z "$CRUSER" ]
    then
        git remote add crremote ssh://review.aquarios.org:29418/$GERRIT_REMOTE
    else
        git remote add crremote ssh://$CRUSER@review.aquarios.org:29418/$GERRIT_REMOTE
    fi
